...
In order to run this plugin, **a mysql database instance with a password is required**. A `docker-compose.yml` that sets that up for you can be found in the example server. If you wish to set it up manually, enter the details into the plugin's `config.yml` file. All the tables will be created automatically. If you're using the template server, no other configuration is required.
